    ...
    bool doLinearize)
{
    ...
    maxlumsofar = 0.0f; 
    luminance =  0.0f; 
    lightpos_int_tex = glm::uvec2(0u , 0u);
    if (isHDR) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                for (int c = 0; c < channelCount; c++) {
                    ...
                    int linIndex = (y * width + x) * channelCount;
                    float  u = x / width;
                    float v = y / height;


                    float* pixels = static_cast<float*>(rawPixels);
                    float mypixel = pixels[linIndex + c];
                    if (c == 0) {
                        luminance += mypixel * 0.2126;
                    }
                    else if (c == 1) {
                        luminance += mypixel * 0.7152;
                    }
                    else if (c == 2) {
                        luminance += mypixel * 0.0722;

                    }

                }
                if (maxlumsofar < luminance) {
                    maxlumsofar = luminance;
                    lightpos_int_tex = glm::uvec2(x, y);
                }
            }

        }
    }
    lightpos_3D = glm::vec3(0.0f , 0.0f ,0.0f );



    lightpos_int_tex.y = (unsigned int)(height-1) -  lightpos_int_tex.y;  

    float u = (float)lightpos_int_tex.x / (float)width;
    float v = (float)lightpos_int_tex.y / (float)height;  
    

    float pi = 3.14159265f;
    float alpha = 2 * pi *u ; 
    float beta = pi * v;
    float dirx = -sin(beta)*cos(alpha);
    float diry = cos(beta);  
    float dirz = -sin(beta) * sin(alpha);

    lightpos_3D = glm::vec3(dirx , diry , dirz );
